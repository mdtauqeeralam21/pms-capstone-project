...
export class MyServiceService {
  ...
  getAllPatients(): Observable<Patientinfo[]> {
    return this.http.get<Patientinfo[]>(
      `http://localhost:9001/patient-info-service/patients`
    );
  }

  getAllAvailability(): Observable<Physician[]> {
    return this.http.get<Physician[]>(
      'http://localhost:9001/physician-availability-service/physician-availability'
    );
  }
  updatePhysician(
    physicianId: number,
  ...
  getAllNurses() {
    return this.http.get<any>('http://localhost:9001/auth0-service/pms/nurses');
  }

  getAllAdmin() {
    return this.http.get<any>('http://localhost:9001/auth0-service/pms/admin');
  }
}
